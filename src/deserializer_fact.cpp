...
RrDeserializerFact::get_deserializers(std::shared_ptr<rclcpp::Node> node) {
  ...
  // add deserializers to the list, note that they will need to be in the same
  // order as the returned clients above
  std::vector<rclcpp::ClientBase::SharedPtr> rv = {
      // key = 0
      static_cast<rclcpp::ClientBase::SharedPtr>(
          node->create_client<rr_interfaces::srv::Joy>(
              rr_constants_state_mgr::STATE_JOY_REQ.c_str()))};
